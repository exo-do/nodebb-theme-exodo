...
<ul component="category" class="category-topics" id="topics-container" itemscope itemtype="http://www.schema.org/ItemList" data-nextstart="{nextStart}">
	...
	<!-- BEGIN topics -->
	<li component="category/topic" class="category-item {function.generateTopicClass} <!-- IF topics.ignored --> ignored <!-- ENDIF topics.ignored --><!-- IF topics.userParticipated --> participated <!-- ENDIF topics.userParticipated --><!-- IF topics.isHot --> hot <!-- ENDIF topics.isHot -->" data-tid="{topics.tid}" data-index="{topics.index}" data-cid="{topics.cid}" itemprop="itemListElement">
		...
		<div class="panel panel-default topic-row clearfix">
			<div class="threadinfo">
				...
				<div class="threaddetails">
					...
					<a href="{function.goLastPostButton}"><i class="fa fa-sign-in" title="Ir al ultimo post No Leído">&nbsp;</i></a>
				</div>
			...
			<dl class="threadlastpost hidden-xs">
				...
				<dd>
					<a href="{config.relative_path}/topic/{topics.slug}/{topics.teaser.index}" title="Ir al último mensaje" class="lastpost">
						<span>[[global:replied_ago, <span class="timeago" title="{topics.teaser.timestamp}"></span>]]<!--{function.humanReadableDateLast}--></span> <i class="fa fa-long-arrow-right"></i>
					</a>
